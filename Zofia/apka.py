...
def filter_data_by_user_id(df):
    ...
    if st.session_state.user_id in df['user_id'].values:
        ...
        return filtered_df
         # Wyświetlanie informacji w pojedynczych liniach
        # st.subheader("Podstawowe informacje o użytkowniku")
        # user_data = filtered_df.iloc[0]  # Pobranie pierwszego rekordu (zakładamy, że user_id jest unikalny)
        
        # gender_map = {1: "Mężczyzna", 0: "Kobieta"}
        # gender = gender_map.get(user_data['gender'], "Nieznana")

        # st.write(f"**Wiek:** {user_data['age']} lat")
        # st.write(f"**Płeć:** {gender}")
        # st.write(f"**Wzrost:** {user_data['height']} cm")
        # st.write(f"**Waga:** {user_data['weight']} kg")
...
if __name__ == "__main__":
        ...
        # calories_by_activity = df.groupby("activity_trimmed")["Calories_per_reading"].sum().reset_index()

        # st.subheader("Wykres spalonych kalorii według typu aktywności")
        # fig, ax = plt.subplots()
        # ax.bar(calories_by_activity["activity_trimmed"], calories_by_activity["Calories_per_reading"], color="skyblue")
        # ax.set_xlabel("Typ aktywności")
        # ax.set_ylabel("Spalone kalorie")
        # ax.set_title("Kalorie według aktywności")
        # st.pyplot(fig)
